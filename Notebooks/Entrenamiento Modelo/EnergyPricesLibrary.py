...
def sMAPE(y_true,y_pred):
    sum_numerator   = np.absolute(y_pred-y_true)
    sum_denominator = np.clip(np.absolute(y_true)+np.absolute(y_pred), np.finfo(np.float32).eps, np.inf)
    sum_factor = np.sum(sum_numerator/sum_denominator)
...
def SplitTimeseriesMultipleTimesBackAhead_differentTimes(df_x,df_y,day='Monday',start_date_train='2000-02-01',start_date_val='2020-01-01',
    ...
    if day == 'All':
        ...
        df_x = df_x[df_x['day_of_week'] == day]
        df_y = df_y[df_y['day_of_week'] == day]
...
def SplitTimeseriesMultipleTimesBackAhead_DifferentTimes_Images(df_x,df_y,day='All',start_date_train='2000-02-01',start_date_val='2020-01-01',
                                                         start_date_test='2020-04-01',end_date_test='2020-05-01',n_steps_out=1,
                                                         n_steps_in = 1, overlap = 1, output_features=None,input_features=None,
                                                         IMG_HEIGHT=635,IMG_WIDTH=460):
